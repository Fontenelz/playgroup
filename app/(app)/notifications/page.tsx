...
import { ChevronLeft, CheckCheck, Bell, BellOff } from 'lucide-react'
import { MOCK_NOTIFICATIONS } from '@/data/mock'
import { cn } from '@/lib/utils'
import type { Notification } from '@/types/app.types'

// Enrich mock with more variety
const EXTRA_NOTIFICATIONS: Notification[] = [
  {
    id: 'notif-4', user_id: 'user-matheus', type: 'event_reminder',
    title: '⏰ Lembrete: jogo amanhã',
    body: 'Futebol Quinta às 20h. Você está confirmado! Campo São Luís FC.',
    data: { groupId: 'group-futebol', eventId: 'event-1' },
    is_read: true, created_at: new Date(Date.now() - 26 * 3600000).toISOString(),
  },
  {
    id: 'notif-5', user_id: 'user-matheus', type: 'payment_overdue',
    title: '⚠️ Pagamento em atraso',
    body: 'Lucas Ferreira está com mensalidade de julho em atraso há 5 dias.',
    data: { groupId: 'group-futebol' },
    is_read: true, created_at: new Date(Date.now() - 2 * 24 * 3600000).toISOString(),
  },
  {
    id: 'notif-6', user_id: 'user-matheus', type: 'event_created',
    title: '📅 Novo evento criado',
    body: 'Beach Vôlei Sábado foi adicionado ao grupo Beach Tennis Verão.',
    data: { groupId: 'group-beach', eventId: 'event-3' },
    is_read: true, created_at: new Date(Date.now() - 3 * 24 * 3600000).toISOString(),
  },
  {
    id: 'notif-7', user_id: 'user-matheus', type: 'member_joined',
    title: '👋 Diego entrou no grupo',
    body: 'Diego Martins aceitou o convite para Futebol Quinta São Luís.',
    data: { groupId: 'group-futebol' },
    is_read: true, created_at: new Date(Date.now() - 4 * 24 * 3600000).toISOString(),
  },
  {
    id: 'notif-8', user_id: 'user-matheus', type: 'event_cancelled',
    title: '❌ Evento cancelado',
    body: 'Vôlei na Praia de quinta foi cancelado pelo organizador.',
    data: { groupId: 'group-volei' },
    is_read: true, created_at: new Date(Date.now() - 6 * 24 * 3600000).toISOString(),
  },
  {
    id: 'notif-9', user_id: 'user-matheus', type: 'ranking_update',
    title: '📈 Você subiu no ranking',
    body: 'Você está em 1º lugar em presenças no Futebol Quinta São Luís!',
    data: { groupId: 'group-futebol' },
    is_read: true, created_at: new Date(Date.now() - 7 * 24 * 3600000).toISOString(),
  },
]

const ALL_NOTIFICATIONS = [...MOCK_NOTIFICATIONS, ...EXTRA_NOTIFICATIONS]
