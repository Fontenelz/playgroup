...
export function CreateEventClient({ group }: { group: GroupData }) {
  ...
  function toggleWeekDay(day: WeekDay) {
    setForm((f) => {
      ...
      if (has && f.weekDays.length === 1) return f
  ...
  function handleSubmit() {
    startTransition(async () => {
      const result = await createEvent(group.id, {
        date:                form.date,
        startTime:           form.startTime,
        endTime:             form.endTime,
        recurrence:          form.recurrence,
        weekDays:            form.weekDays,
        seriesEnd:           form.seriesEnd,
        locationName:        form.locationName,
        locationAddress:     form.locationAddress,
        maxParticipants:     form.maxParticipants,
        monthlySlots:        form.monthlySlots,
        monthlyConfirmHours: form.monthlyConfirmHours,
        eventFee:            form.useGroupFee ? (group.per_event_fee?.toString() ?? '') : form.eventFee,
        notes:               form.notes,
      })
    ...
    <div className="min-h-screen flex flex-col max-w-lg mx-auto">
      ...
      <div className="sticky top-0 z-40 bg-slate-950/95 backdrop-blur-md border-b border-slate-800/60 px-4 pt-4 pb-4 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <SportIcon sport={group.sport as Parameters<typeof SportIcon>[0]['sport']} size="sm" />
            <div>
      ...
      <div className="flex-1 overflow-hidden">
        <AnimatePresence mode="wait" custom={dir}>
          ...
          >
            {step === 1 && <StepWhen form={form} set={set} toggleWeekDay={toggleWeekDay} />}
            {step === 2 && <StepWhere form={form} set={set} />}
            {step === 3 && <StepSlots form={form} set={set} />}
            {step === 4 && (
              <StepDetails
                form={form}
                set={set}
            ...
            {WEEKDAYS.map(({ id, short, label }) => {
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    toggleWeekDay(id)
                    if (!form.weekDays.includes(id)) set('date', nextDateForWeekday(id))
                  }}
                  className={cn(
